...
stmt = plpy.prepare("SELECT pgpyml_try.predict($1, $2)", ['text', 'real[]'])
results =  plpy.execute(stmt, [model_path, [features]], 1)
...
if model_key in GD:
    clf = GD[model_key]
    plpy.notice('clf address:', id(clf))
else:
    from joblib import load
    clf = load(model_path) 
    GD[model_key] = clf
...
selected_values = plpy.execute('SELECT * FROM pgpyml_try.predict(%s, (SELECT ARRAY[[%s]] FROM %s WHERE id = %d))' % (plpy.quote_literal(model_path), features, plpy.quote_ident(table_name), id), 1)
...
* model slice
*/
CREATE OR REPLACE FUNCTION model_slice(model_path text) RETURNS BYTEA AS
$$
